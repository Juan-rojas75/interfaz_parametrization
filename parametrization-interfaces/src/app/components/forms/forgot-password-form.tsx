      ...
      try {
        if (formData) {
            ...
            if (email && password && passwordConfirm && password === passwordConfirm) {
                await authContext.recovery_pass(email, password,passwordConfirm);
                showToast("¡Éxito!", "Recuperación realizada correctamente.", 4000);
          ...
          showToast("Error!", "No se completo la recuperación.", 4000);
          hideLoader();
      }
    };
      ...
      <article className="min-h-screen flex items-center justify-center bg-secondary-200">
        <article className="flex flex-row bg-white backdrop-blur-3xl rounded-lg">
          <section className="px-10 py-6 text-left max-w-96 text-primary-950 hidden md:block">
            ...
            <p className="text-xl">
              Podras recuperarla con tu correo electr&oacute;nico.
            </p>
          </section>
          ...
          {errorMessage && <p className="text-red-500">{errorMessage}</p>}
            <div className="mb-4">
              ...
              <input
                ...
                id="email"
                name="email"
                className="w-full p-2 text-sm text-primary-950 border border-secondary-200 rounded-md focus:outline-none focus:ring-2 focus:ring-secondary-200"
            ...
            <button
              type="submit"
              className="w-full bg-white text-primary-950 py-2 px-4 rounded-md hover:bg-secondary-300 focus:outline-none focus:ring-2 focus:ring-secondary-200"
            >
              Continuar
            </button>
            <div className="mt-6 text-center">
              <Link href="/auth/login" className="text-sm text-secondary-300 hover:underline">Ingreso de sesión</Link>
            </div>
          </form>
        </article>
      </article>
